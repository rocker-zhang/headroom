...
class ContentRouterConfig:
    ...
    #       falls back to the protect_recent_reads_fraction heuristic, which is
    #       itself 0.0 by default, so the net default is "protect everything").
    ...
    # conservative mode that skips ML-based (non-deterministic) Kompress on tool
    # outputs. This prevents turn-to-turn compression variance from busting the
    # upstream provider's prefix cache while the session is building its prefix.
...
class ContentRouter(Transform):
    ...
    def _compress_conservative(
        ...
        non-deterministic Kompress output varying turn-to-turn and busting the
        upstream provider's prefix cache.
        ...
        TabularCompressor. Stochastic strategies skipped: Kompress (ML-based
        ModernBERT), and TEXT/MIXED which route through Kompress internally.
        """
        ...
        # If routing selected a Kompress-based (non-deterministic) strategy,
        # pass through unchanged. Kompress can produce different compressed
        # output across calls for the same input, which invalidates the
        # upstream prefix cache. TEXT and MIXED also fan out to Kompress, so
        # they are treated as conservatively non-compressible here.
        if strategy in (
        ...
        # Tabular, CodeAware), compression is deterministic — allow it.
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        # Approximate session turn count: each turn = 1 user + 1 assistant
        # message. Used for stability pinning and stale-read decisions.
        ...
        #   stale_read_compress_after_turns=N → protect last N turns (2*N msgs),
        #     allow compression for older Read/Glob outputs.
        #   stale_read_compress_after_turns=0   → fall back to fraction logic
        #     (default 0.0 fraction → protect all → byte-identical old behavior).
        #   stale_read_compress_after_turns=999 → protect everything (explicit
        ...
        # 0 (default) → never in stability phase → byte-identical old behavior.
        ...
        for i, message in enumerate(messages):
            ...
            # In the stability phase, mark tool messages for conservative
            # (no-Kompress) mode so early-turn tool output stays byte-stable.
        ...
        if pending_tasks:
            ...
            for (slot_idx, task_content, _, _, content_key, task_conservative), (
                result,
                compress_ms,
    ...
    def _compress_block_content(
        ...
        # Cache miss — run full compression (or conservative in stability phase)
        if route_counts is not None:
        ...
        # Didn't compress enough. In conservative (stability) mode, do NOT
        # mark_skip — this content may be compressible via Kompress once the
        # stability phase ends. Only skip genuine non-compressibles (full mode).
        if not conservative:
            self._cache.mark_skip(content_key)
        if route_counts is not None:
            route_counts["ratio_too_high"] = route_counts.get("ratio_too_high", 0) + 1
